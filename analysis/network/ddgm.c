...
void    readValue(char *fn1)
{
	...
	for(i = 0; i < Dn; i++){
		...
		for(j = 0; j < 1024; j++) if((MatE[i][j] = getc(fp)) == '\n') break; 
		MatE[i][j] = '\0'; 
	}
	fclose(fp);
//	printf("%d %d\n", Dm, Dn);
...
void    readUid(char *fn1)
{
	...
	for(i = 0; i < Dm; i++){
		fscanf(fp, "%d ", &VecD[i]); VecD[i]--; 
		MatF[i] = (char *) malloc(sizeof(char)*1023);
		for(j = 0; j < 1024; j++) if((MatF[i][j] = getc(fp)) == '\n') break; 
		MatF[i][j] = '\0'; 
	}
	fclose(fp);
//	printf("%d\n", Dm);
...
void	initData()
{
	...
	for(j = 0, Dc = -Dm-1; j < Dm; j++) VecC[j] = Dc; 
	...
	for(j = 0; j < Dm+3*Dn; j++) MatW[j] = (double *) malloc(sizeof(double)*2);; 
	MatL = (int **) malloc(sizeof(int *)*(4*Dn));
	for(j = 0; j < 4*Dn; j++) MatL[j] = (int *) malloc(sizeof(int)*2);; 
//	printf("init OK\n");
}
void	calValue()
{
	...
	for(i = 0; i < Dn; i++){ 
		...
		*/
		if(u == Dc && v == Dc){ 
			VecL[i] = -(x+1); VecR[i] = -(y+1); VecS[i] = 2; 
		} else if(u == Dc && v >= 0){ 
			VecL[i] = -(x+1); VecR[i] = v; VecS[i] = 1+VecS[v]; 
			for(k = 0; k < Dm; k++) if(VecC[k] == v) VecC[k] = i; 
		} else if(u >= 0 && v == Dc){ 
			VecL[i] = u; VecR[i] = -(y+1); VecS[i] = VecS[u]+1; 
			for(k = 0; k < Dm; k++) if(VecC[k] == u) VecC[k] = i; 
		} else if(u != v){ 
//			if(VecS[u] >= VecS[v]){ VecL[i] = u; VecR[i] = v; } else{ VecL[i] = v; VecR[i] = u; }
			VecL[i] = u; VecR[i] = v; VecS[i] = VecS[u]+VecS[v]; 
			for(k = 0; k < Dm; k++) if(VecC[k] == u || VecC[k] == v) VecC[k] = i; 
		} 
	}
//	printf("cal OK\n");
...
double	plotValue(int i, int s, int e)
{
	...
	h = Dm+(3*i); 
	MatW[h][0] = a; MatW[h][1] = z;
	MatW[h+1][0] = b; MatW[h+1][1] = z;
	MatW[h+2][0] = (a+b)/2.0; MatW[h+2][1] = z;
	MatL[Ds][0] = h; MatL[Ds++][1] = j;
	MatL[Ds][0] = h; MatL[Ds++][1] = h+2;
	MatL[Ds][0] = h+1; MatL[Ds++][1] = k;
	MatL[Ds][0] = h+1; MatL[Ds++][1] = h+2;
	return((a+b)/2.0); 
}
void	printValue(char *fn1, int width)
{
	...
	double	v, w; 
	for(i = 0; i < Dm+3*Dn; i++) if(i == 0) v = w = MatW[i][0]; else if(v > MatW[i][0]) v = MatW[i][0]; else if(w < MatW[i][0]) w = MatW[i][0];
	for(i = 0, w = w-v; i < Dm+3*Dn; i++) MatW[i][0] = (0.98*x*((MatW[i][0]-v)/w))+(0.01*x);
	for(i = 0; i < Dm+3*Dn; i++) MatW[i][1] = -MatW[i][1]; 
	for(i = 0; i < Dm+3*Dn; i++) if(i == 0) v = w = MatW[i][1]; else if(v > MatW[i][1]) v = MatW[i][1]; else if(w < MatW[i][1]) w = MatW[i][1];
	for(i = 0, w = w-v; i < Dm+3*Dn; i++) MatW[i][1] = (0.98*y*((MatW[i][1]-v)/w))+(0.01*y);
	fp = fopen(fn1, "w"); 
